...
class BlockingQueriesAnalyzer:
    ...
    async def get_blocking_queries(self) -> str:
        ...
        try:
            ...
            recommendations = await self._generate_enhanced_recommendations(
                blocking_data, summary, session_termination_recs, lock_timeout_suggestions,
                historical_analysis, lock_escalation_info, query_pattern_analysis
            )
    ...
    async def _generate_lock_timeout_suggestions(self, blocking_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if rows:
                for row in rows:
                    current_settings[row.cells["name"]] = {
                        "value": row.cells["setting"],
                        "unit": row.cells["unit"]
                    }
            ...
            if wait_times:
                ...
                suggestions = {
                    "current_settings": current_settings,
                    "recommendations": {
                        "lock_timeout": {"suggested_value": "60s", "reason": "Standard recommendation for OLTP workloads"},
                        "statement_timeout": {"suggested_value": "600s", "reason": "Standard recommendation to prevent runaway queries"},
                        "idle_in_transaction_session_timeout": {"suggested_value": "300s", "reason": "Prevent idle transactions from holding locks"}
                    }
                }
        ...
        try:
            ...
            for block in blocking_data:
                ...
                if blocking_query:
                    ...
                    batch_keywords = ['INSERT INTO', 'UPDATE', 'DELETE FROM', 'COPY', 'CREATE INDEX', 'VACUUM', 'ANALYZE']
    ...
    async def _generate_enhanced_recommendations(
        ...
        if session_termination_recs:
            ...
            for rec in session_termination_recs[:3]:  # Top 3 recommendations
                recommendations.append(
                    f"   • {rec['priority']}: Terminate PID {rec['target_pid']} - {rec['reason']}"
                )
        ...
        if lock_escalation_info.get("detected_escalations"):
            ...
            recommendations.extend([f"   • {rec}" for rec in lock_escalation_info["recommendations"][:2]])
